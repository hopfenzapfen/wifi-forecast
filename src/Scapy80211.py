...
class Scapy80211():
    ...
    def  __init__(self,intf='mon0',ssid='test1',\
    ...
    def Beacon(self,count=10,ssid='',dst='ff:ff:ff:ff:ff:ff', bssid='00:11:22:33:44:55'):
      if not ssid: ssid=self.ssid
      if not bssid: bssid=self.bssid
      ...
      pkt = RadioTap()\
        /Dot11(type=0,subtype=8,addr1=dst,addr2=self.source,addr3=bssid)\
        /beacon/essid/rates/dsset/tim

      print '[*] 802.11 Beacon: SSID=[%s], count=%d' % (ssid,count)
      try:
        sendp(pkt,iface=self.intfmon,count=count,inter=0.1,verbose=1)
      except:
        raise
...
if __name__ == "__main__":
    ...
    print """
        ...
        [*] Assumes '{0}' is your wireless NIC!
        [*] Author: Joff Thyer, 2014
        """.format(iface)
    sdot11 = Scapy80211(intf=iface)

    for i in range(0, 5):
        sdot11.Beacon(count=count, bssid="00:11:22:33:44:55", ssid="1 We love")
        sdot11.Beacon(count=count, bssid="00:11:22:33:44:56", ssid="2 chocolate")
    # sdot11.ProbeReq()
